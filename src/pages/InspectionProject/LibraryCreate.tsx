...
import { PlusOutlined, DeleteOutlined, SearchOutlined, EyeOutlined, DownloadOutlined } from '@ant-design/icons'
import { useNavigate } from 'react-router-dom'

/**
 * 页面组件：新增检验项目（独立页面）
 * 职责：
 * - 以全页形式展示多页签表单，适合填写大量内容；
 * - 保存时将布尔字段映射为 0/1，并写入 localStorage；
 * 入参：无
 * 出参：React 组件（JSX.Element）
 */
/**
 * 页面组件：新增检验项目（独立页面）
 * 布局：顶部“基本信息”独立块（横向表单），下方为其他模块的Tabs
 * 入参：无
 * 出参：React 组件（JSX.Element）
 */
const InspectionProjectLibraryCreate: React.FC = () => {
  ...
  /** 使用组织列表状态 */
  const [orgList, setOrgList] = useState<{ id: string, orgCode?: string, orgName?: string }[]>([])
  /** 使用组织表格选中项 */
  const [selectedOrgKeys, setSelectedOrgKeys] = useState<React.Key[]>([])

  /**
   * 样本要求列表（扩展字段）
   * 说明：根据需求新增并排序字段，支持下拉与文本输入
   * 入参：无
   * 出参：React 状态（数组）
   */
  ...
  /** 生成唯一编码（基于localStorage数量） */
  const generateCode = (): string => {
    const cached = localStorage.getItem('inspectionProjects')
    const list = cached ? JSON.parse(cached) : []
    const prefix = 'IPJ-'
    const seq = String((Array.isArray(list) ? list.length : 0) + 1).padStart(6, '0')
    return `${prefix}${seq}`
  }
  ...
  const handleMethodologyChange = (v: string): void => {
    ...
    message.warning('方法学变更可能影响单位/参考区间与配置，请复核相关设置')
  }
  ...
  React.useEffect(() => {
    form.setFieldsValue({
      code: generateCode(),
      status: true,
      requiresBioinformatics: false,
      organizationId: defaultOrgId,
    })
  }, [])

  /** 保存：写入localStorage并返回列表 */
  ...
  /**
   * 渲染：使用组织
   * 说明：在“报告模板”右侧新增一个页签，用于维护可使用本检验项目的组织列表。
   * 列表字段：组织编码、组织名称；顶部提供“新增”和“取消分派”两个按钮。
   * 入参：无
   * 出参：JSX.Element
   */
  const renderOrganizations = (): JSX.Element => {
    /**
     * 方法：更新某一行的字段
     * 入参：idx 行索引；field 字段名；value 字段值
     * 出参：无
     */
    const updateRow = (idx: number, field: 'orgCode' | 'orgName', value: string): void => {
      setOrgList(prev => prev.map((it, i) => i === idx ? { ...it, [field]: value } : it))
    }
    ...
    const handleAddOrg = (): void => {
      setOrgList(prev => [{ id: `${Date.now()}`, orgCode: '', orgName: '' }, ...prev])
    }
    ...
    const handleUnassign = (): void => {
      if (selectedOrgKeys.length === 0) { message.info('请先选择需要取消分派的组织'); return }
      setOrgList(prev => prev.filter(it => !selectedOrgKeys.includes(it.id)))
      setSelectedOrgKeys([])
      message.success('已取消分派选中组织')
    }

    const columns = [
      { title: '组织编码', dataIndex: 'orgCode', key: 'orgCode', width: 200, render: (_: any, row: any, idx: number) => (
        <Input placeholder="请输入组织编码" value={row.orgCode} onChange={e => updateRow(idx, 'orgCode', e.target.value)} />
      ) },
      { title: '组织名称', dataIndex: 'orgName', key: 'orgName', width: 240, render: (_: any, row: any, idx: number) => (
        <Input placeholder="请输入组织名称" value={row.orgName} onChange={e => updateRow(idx, 'orgName', e.target.value)} />
      ) },
    ]
    ...
    return (
      <>
        <Space style={{ marginBottom: 8 }}>
          <Button type="primary" onClick={handleAddOrg}>新增</Button>
          <Button danger onClick={handleUnassign}>取消分派</Button>
        </Space>
        <Table
          rowKey="id"
          size="small"
          ...
          rowSelection={{
            ...
            onChange: (keys) => setSelectedOrgKeys(keys),
          }}
        />
      </>
    )
        ...
        <Form form={form} layout="horizontal" labelAlign="right" labelCol={{ span: 6 }} wrapperCol={{ span: 18 }}>
          ...
          <Tabs
            ...
            items={[
              ...
              { key: 'org', label: '使用组织', children: renderOrganizations() },
            ]}
          />
        </Form>
      </Card>
...
export default InspectionProjectLibraryCreate